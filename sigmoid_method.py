...
def caculate_sigmoid(model, x, delta_x, y, steps=5, alpha=0.0025, lbda=0.01, kind=2):
    ...
    for _ in range(steps):
        ...
        loss.backward()
        optimizer.step()
    sigmoid_output = sigmoid_outputs[-1]
    return sigmoid_output


def exp(model, x, delta_x, y, steps=5, alpha=0.0025, lbda=0.01, kind=2):
    sigmoid_output = caculate_sigmoid(model, x.unsqueeze(0), delta_x.unsqueeze(
        0), y.argmax(-1).unsqueeze(0), steps, alpha, lbda, kind)
    return torch.norm(delta_x * sigmoid_output).item(), delta_x * sigmoid_output
